...
class Prompts {
        ...
        const answers = await inquirer_1.default.prompt([
            {
                ...
                validate: (input) => {
                    const result = validation_1.Validators.companyName(input);
    ...
    static async state() {
        ...
        const initial = await inquirer_1.default.prompt([
            {
                type: 'list',
                name: 'state',
        ...
        if (initial.state === 'all') {
            ...
            const fullSelection = await inquirer_1.default.prompt([
                {
                    type: 'list',
                    name: 'state',
     ...
     */
    static async numberOfShareholders() {
        const answers = await inquirer_1.default.prompt([
            {
                type: 'number',
                name: 'count',
                ...
                validate: (input) => {
                    if (input < 1)
                        return 'Must have at least 1 owner';
                    if (input > 100)
                        return 'Maximum 100 owners allowed';
                    return true;
                }
            }
        ]);
     ...
     */
    static async shareholderInfo(index, totalCount) {
        ...
        const answers = await inquirer_1.default.prompt([
            {
                ...
                validate: (input) => {
                    const result = validation_1.Validators.personName(input);
            ...
            },
            {
                ...
                validate: (input) => {
                    const result = validation_1.Validators.ownershipPercentage(input);
            ...
            },
            {
                type: 'password',
                name: 'taxId',
                message: (answers) => answers.idType === 'ssn' ? 'SSN (XXX-XX-XXXX):' : 'EIN (XX-XXXXXXX):',
                mask: '*',
                validate: (input, answers) => {
                    const result = answers.idType === 'ssn' ?
                        validation_1.Validators.ssn(input) :
                        validation_1.Validators.ein(input);
    ...
    static async registeredAgent() {
        ...
        const agentAnswers = await inquirer_1.default.prompt([
            {
                type: 'input',
                name: 'name',
                message: 'Registered agent name or company:',
                validate: (input) => {
                    const result = validation_1.Validators.required(input, 'Registered agent name');
            ...
            },
            {
                ...
                name: 'state',
                message: 'State:',
                validate: (input) => input.length === 2 || 'Use 2-letter state code'
            },
            {
                type: 'input',
                name: 'zipCode',
     ...
     */
    static async confirmPayment(total) {
        const answers = await inquirer_1.default.prompt([
            {
                type: 'confirm',
                name: 'confirm',
     ...
     */
    static async selectFieldToEdit(fields) {
        const answers = await inquirer_1.default.prompt([
            {
                type: 'list',
                name: 'field',
                message: 'Which field would you like to edit?',
                choices: [...fields, new inquirer_1.default.Separator(), 'Go back']
            }
        ]);
