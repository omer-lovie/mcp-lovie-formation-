...
exports.formationSetCompanyAddressTool = exports.formationCheckNameTool = exports.formationSetCompanyNameTool = exports.formationSetEntityEndingTool = exports.formationSetCompanyTypeTool = exports.formationSetStateTool = void 0;
...
const INCORPORATION_PROCESS_EXPLANATION = {
    ...
    incorporator: {
        name: types_1.DEFAULT_INCORPORATOR.name,
...
const handleFormationSetState = async (args, store) => {
    const sessionId = args.sessionId;
    const state = args.state;
    if (!['DE', 'WY'].includes(state)) {
        throw (0, errors_1.validationError)('state', 'Only Delaware (DE) and Wyoming (WY) are currently supported');
    }
    const session = await (0, session_1.loadSession)(sessionId, store);
    // Get available company types for this state
    const availableTypes = types_1.STATE_COMPANY_TYPES[state];
    const businessDescription = session.companyDetails?.businessDescription;
    session.companyDetails = {
        ...session.companyDetails,
        state,
    };
    session.currentStep = types_1.FormationStep.STATE_SELECTED;
    ...
    const companyTypes = availableTypes.map(type => ({
        ...
        description: types_1.COMPANY_TYPE_DESCRIPTIONS[type],
    }));
    ...
    return {
        success: true,
        state,
        stateDescription: types_1.STATE_DESCRIPTIONS[state],
        companyTypes,
        recommendation,
        message: state === 'WY'
...
exports.formationSetCompanyTypeTool = {
...
const handleFormationSetCompanyType = async (args, store) => {
    const sessionId = args.sessionId;
    const companyType = args.companyType;
    if (!['LLC', 'C-Corp'].includes(companyType)) {
        throw (0, errors_1.validationError)('companyType', 'Must be LLC or C-Corp');
    }
    const session = await (0, session_1.loadSession)(sessionId, store);
    ...
    if (state) {
        ...
        if (!availableTypes.includes(companyType)) {
            throw (0, errors_1.validationError)('companyType', `${companyType} is not available in ${state}. Available types: ${availableTypes.join(', ')}`);
        }
    }
    ...
    if (state === 'DE') {
        session.incorporator = types_1.DEFAULT_INCORPORATOR;
    }
    session.currentStep = types_1.FormationStep.TYPE_SELECTED;
    await store.save(session);
    const entityEndings = types_1.ENTITY_ENDINGS[companyType];
    ...
    return {
        ...
        state,
        entityEndings,
        description: types_1.COMPANY_TYPE_DESCRIPTIONS[companyType],
        upgradeNote,
...
exports.formationSetEntityEndingTool = {
...
const handleFormationSetEntityEnding = async (args, store) => {
    const sessionId = args.sessionId;
    const entityEnding = args.entityEnding;
    const session = await (0, session_1.loadSession)(sessionId, store);
    const companyType = session.companyDetails?.companyType;
    if (!companyType) {
        throw (0, errors_1.validationError)('companyType', 'Company type must be set first');
    }
    if (!(0, validation_1.validateEntityEnding)(entityEnding, companyType)) {
        const validEndings = types_1.ENTITY_ENDINGS[companyType].join(', ');
        throw (0, errors_1.validationError)('entityEnding', `Invalid entity ending for ${companyType}. Valid options: ${validEndings}`);
    }
    session.companyDetails = {
        ...session.companyDetails,
        entityEnding,
    };
    session.currentStep = types_1.FormationStep.ENDING_SELECTED;
    await store.save(session);
    return {
        success: true,
        entityEnding,
...
exports.formationSetCompanyNameTool = {
...
const handleFormationSetCompanyName = async (args, store) => {
    const sessionId = args.sessionId;
    const baseName = args.baseName;
    if (!baseName || baseName.length < 3) {
        throw (0, errors_1.validationError)('baseName', 'Company name must be at least 3 characters');
    }
    if (baseName.length > 200) {
        throw (0, errors_1.validationError)('baseName', 'Company name must be at most 200 characters');
    }
    const session = await (0, session_1.loadSession)(sessionId, store);
    const entityEnding = session.companyDetails?.entityEnding;
    if (!entityEnding) {
        throw (0, errors_1.validationError)('entityEnding', 'Entity ending must be set first');
    }
    const fullName = `${baseName} ${entityEnding}`;
    if (fullName.length > 245) {
        throw (0, errors_1.validationError)('fullName', 'Full company name exceeds Delaware limit of 245 characters');
    }
    session.companyDetails = {
        ...session.companyDetails,
        baseName,
    ...
    session.currentStep = types_1.FormationStep.NAME_SET;
    await store.save(session);
    return {
        success: true,
        baseName,
...
exports.formationCheckNameTool = {
...
const handleFormationCheckName = async (args, store) => {
    const sessionId = args.sessionId;
    const session = await (0, session_1.loadSession)(sessionId, store);
    ...
    const companyType = session.companyDetails?.companyType;
    if (!companyName) {
        throw (0, errors_1.validationError)('companyName', 'Company name must be set first using formation_set_company_name');
    }
    try {
        ...
        session.currentStep = types_1.FormationStep.NAME_CHECKED;
        await store.save(session);
        return {
            available: result.available,
            companyName,
...
exports.formationSetCompanyAddressTool = {
...
const handleFormationSetCompanyAddress = async (args, store) => {
    const sessionId = args.sessionId;
    const source = args.source;
    const virtualPostMailInterested = args.virtualPostMailInterested;
    ...
    if (source === 'own') {
        ...
        if (!addressInput) {
            throw (0, errors_1.requiredFieldError)('address');
        }
        if (!addressInput.street1)
            throw (0, errors_1.requiredFieldError)('address.street1');
        if (!addressInput.city)
            throw (0, errors_1.requiredFieldError)('address.city');
        if (!addressInput.state)
            throw (0, errors_1.requiredFieldError)('address.state');
        if (!addressInput.zipCode)
            throw (0, errors_1.requiredFieldError)('address.zipCode');
        companyAddress = {
            source: 'own',
        ...
        session.currentStep = types_1.FormationStep.COMPANY_ADDRESS_SET;
        await store.save(session);
        return {
            success: true,
            companyAddress,
...
function registerCompanyTools() {
    (0, index_1.registerTool)(exports.formationSetStateTool, handleFormationSetState);
    (0, index_1.registerTool)(exports.formationSetCompanyTypeTool, handleFormationSetCompanyType);
    (0, index_1.registerTool)(exports.formationSetEntityEndingTool, handleFormationSetEntityEnding);
    (0, index_1.registerTool)(exports.formationSetCompanyNameTool, handleFormationSetCompanyName);
    (0, index_1.registerTool)(exports.formationCheckNameTool, handleFormationCheckName);
    (0, index_1.registerTool)(exports.formationSetCompanyAddressTool, handleFormationSetCompanyAddress);
}
