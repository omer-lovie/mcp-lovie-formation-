...
exports.NameCheckClient = void 0;
exports.createNameCheckClient = createNameCheckClient;
...
class NameCheckClient {
    ...
    async checkAvailability(request) {
        return this.executeWithRetry(async () => {
            try {
                const response = await this.axiosInstance.post('/api/v1/check', request);
     ...
     * ```
     */
    async checkNameByType(baseName, companyType) {
     ...
     * ```
     */
    async checkNameWithCode(baseName, entityType, entityEnding) {
        const ending = entityEnding || types_1.DELAWARE_ENTITY_TYPES[entityType].defaultEnding;
        return this.checkAvailability({
            baseName,
            entityType,
            entityEnding: ending,
    ...
    async executeWithRetry(fn) {
        ...
        throw new Error(`Name check failed after ${this.retryConfig.maxAttempts} attempts: ${lastError?.message}`);
    }
    /**
    ...
    handleError(error) {
        if (axios_1.default.isAxiosError(error)) {
            const axiosError = error;
            if (!axiosError.response) {
                return new Error('Network error: Unable to connect to name check service. Please check your internet connection.');
            }
            const status = axiosError.response.status;
            ...
            if (status === 400) {
                return new Error(apiError?.message || 'Invalid request. Please check your input.');
            }
            if (status >= 500) {
                return new Error('Service temporarily unavailable. Please try again in a few moments.');
            }
            return new Error(apiError?.message || 'An unexpected error occurred during name check.');
        }
...
exports.NameCheckClient = NameCheckClient;
/**
 ...
 * ```
 */
function createNameCheckClient() {
...
//# sourceMappingURL=nameCheckClient.js.map