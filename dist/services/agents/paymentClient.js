...
class PaymentClient {
    ...
    async processPayment(request) {
        return this.executeWithRetry(async () => {
            try {
                ...
                const response = await this.axiosInstance.post('/payments/process', sanitizedRequest);
    ...
    async getPaymentStatus(paymentId) {
        try {
            ...
            if (!response.data.success) {
                throw new Error(response.data.error?.message || 'Failed to retrieve payment status');
            }
            return response.data.data;
        }
        catch (error) {
    ...
    async refundPayment(paymentId, amount, reason) {
        return this.executeWithRetry(async () => {
            try {
                const response = await this.axiosInstance.post(`/payments/${paymentId}/refund`, {
                    amount,
                    reason,
                });
                if (!response.data.success) {
                    throw new Error(response.data.error?.message || 'Refund processing failed');
                }
                return response.data.data;
            }
            catch (error) {
     ...
     * ```
     */
    async calculateCost(state, companyType, expedited = false) {
        try {
            const response = await this.axiosInstance.get('/payments/calculate-cost', {
                params: { state, companyType, expedited },
            });
            if (!response.data.success) {
                throw new Error(response.data.error?.message || 'Failed to calculate costs');
            }
            return response.data.data;
        }
        catch (error) {
    ...
    async validatePaymentMethod(paymentMethod) {
        try {
            const response = await this.axiosInstance.post('/payments/validate', { paymentMethod });
            if (!response.data.success) {
     ...
     */
    sanitizePaymentRequest(request) {
    ...
    async executeWithRetry(fn) {
        ...
        throw new Error(`Payment processing failed after ${this.retryConfig.maxAttempts} attempts: ${lastError?.message}`);
    }
    /**
    ...
    handleError(error) {
        if (axios_1.default.isAxiosError(error)) {
            const axiosError = error;
            if (!axiosError.response) {
                return new Error('Network error: Unable to connect to payment service. Please check your internet connection.');
            }
            const status = axiosError.response.status;
            ...
            if (status === 400) {
                return new Error(apiError?.message || 'Invalid payment information. Please check your details.');
            }
            if (status === 402) {
            ...
            if (status === 429) {
                return new Error('Too many payment attempts. Please wait a moment and try again.');
            }
            if (status >= 500) {
                return new Error('Payment service temporarily unavailable. Your card has not been charged. Please try again.');
            }
            return new Error(apiError?.message || 'An unexpected error occurred during payment processing.');
        }
