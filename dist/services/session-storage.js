...
class SessionStorage {
    constructor() {
        ...
        this.storageDir = path_1.default.join(os_1.default.homedir(), '.lovie', 'certificates');
        this.certificateFile = path_1.default.join(this.storageDir, 'certificates.json');
    ...
    ensureStorageDir() {
        if (!fs_1.default.existsSync(this.storageDir)) {
            fs_1.default.mkdirSync(this.storageDir, { recursive: true, mode: 0o700 });
        }
    }
    /**
    ...
    loadAllCertificates() {
        ...
        try {
            const data = fs_1.default.readFileSync(this.certificateFile, 'utf-8');
     ...
     */
    saveAllCertificates(certificates) {
        try {
            // Data already in serializable format (ISO strings)
            fs_1.default.writeFileSync(this.certificateFile, JSON.stringify(certificates, null, 2), { encoding: 'utf-8', mode: 0o600 });
        }
        catch (error) {
     ...
     */
    saveCertificateData(sessionId, downloadUrl, s3Uri, metadata, expiresAt) {
        try {
     ...
     */
    getCertificateDataById(certificateId) {
        try {
            const certificates = this.loadAllCertificates();
     ...
     */
    getAllCertificatesForSession(sessionId) {
        try {
            const certificates = this.loadAllCertificates();
     ...
     * @returns Success status
     */
    invalidateCertificate(certificateId) {
        try {
            const certificates = this.loadAllCertificates();
            const cert = certificates[certificateId];
            if (!cert) {
     ...
     */
    clearCertificateData(sessionId) {
        try {
            const certificates = this.loadAllCertificates();
            let clearedCount = 0;
     ...
     * @returns Success status
     */
    clearCertificateById(certificateId) {
        try {
            const certificates = this.loadAllCertificates();
     ...
     * @returns Number of certificates cleaned up
     */
    cleanupOldCertificates(daysOld = 90) {
        try {
            const certificates = this.loadAllCertificates();
     ...
     * @returns Success status
     */
    updateCertificateUrl(certificateId, newDownloadUrl, newExpiresAt) {
        try {
            const certificates = this.loadAllCertificates();
            const cert = certificates[certificateId];
            if (!cert) {
