 ...
 */
export class DocumentFillerAgent {
  private apiEndpoint: string;
  private timeout: number = 30000; // 30 seconds per SC-006

  constructor(apiEndpoint: string = process.env.DOCUMENT_FILLER_API_URL || '') {
  ...
  ): Promise<AgentResult<DocumentResult>> {
   ...
   */
  private async performDocumentGeneration(
    request: DocumentRequest
   ...
   */
  private getRequiredDocuments(data: FormationData): string[] {
    ...
    switch (companyDetails.type) {
      case CompanyType.LLC:
        documents.push(
          ...
          'Operating Agreement',
          'Initial Members Resolution'
        );
        break;

      case CompanyType.C_CORP:
      case CompanyType.S_CORP:
        documents.push(
        ...
        if (companyDetails.type === CompanyType.S_CORP) {
   ...
   */
  async previewDocuments(data: FormationData): Promise<{
    documents: Array<{
      type: string;
      preview: string;
    }>;
   ...
   */
  private generateDocumentPreview(type: string, data: FormationData): string {
  ...
  async getAvailableTemplates(
    ...
    return this.getRequiredDocuments({
      companyDetails: { state: state as any, type: companyType, name: '' },
      shareholders: [],
      sessionId: '',
      currentStep: 'company_details' as any,
