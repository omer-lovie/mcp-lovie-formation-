 ...
 */
export type USState =
  | 'DE' | 'CA' | 'TX' | 'NY' | 'FL' | 'IL' | 'PA' | 'OH' | 'GA' | 'NC'
  | 'MI' | 'NJ' | 'VA' | 'WA' | 'AZ' | 'MA' | 'TN' | 'IN' | 'MO' | 'MD'
  | 'WI' | 'CO' | 'MN' | 'SC' | 'AL' | 'LA' | 'KY' | 'OR' | 'OK' | 'CT'
  | 'UT' | 'IA' | 'NV' | 'AR' | 'MS' | 'KS' | 'NM' | 'NE' | 'WV' | 'ID'
 ...
 */
export const DELAWARE_ENTITY_TYPES: Record<DelawareEntityType, DelawareEntityInfo> = {
  'C': {
    code: 'C',
    name: 'Corporation',
    requiresEnding: true,
 ...
 */
export const COMPANY_TYPE_TO_DELAWARE: Record<CompanyType, DelawareEntityType> = {
  'LLC': 'Y',
  'C-Corp': 'C',
...
export interface NameCheckRequest {
  baseName: string;
  entityType: DelawareEntityType;  // One of: C, Y, L, P, G, T
  entityEnding: string; // Entity ending (e.g., "LLC", "Inc.", "LP")
  requestId?: string;
}
 ...
 */
export function isNameAvailable(response: NameCheckResponse): boolean {
